...
test('normalizeOptions riporta tutto entro i limiti', () => {
  const o = normalizeOptions({
    ...
    palette: 'inesistente', serpentine: 'si', maxSize: 1,
  });
...
test('processImage rispetta lato massimo, scala e ringrandimento', () => {
  const img = gradient(800, 400);
  const grande = processImage(img, { maxSize: 200, scale: 1 });
  assert.equal(grande.image.width, 200);

  const bloccosa = processImage(img, { maxSize: 200, scale: 4, upscale: true });
  assert.equal(bloccosa.ditherWidth, 50);
  assert.equal(bloccosa.image.width, 200);

  const cruda = processImage(img, { maxSize: 200, scale: 4, upscale: false });
  assert.equal(cruda.image.width, 50);
});
